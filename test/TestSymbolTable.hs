...
testEmptySymbolTable = TestList     [ myAssertEqual "Encoding empty symbol table" symbolTableEnd (writeSymbolTable [])
                                    , myAssertEqual "Decoding empty symbol table (nothing after)" ([], []) (readSymbolTable symbolTableEnd)
                                    , myAssertEqual "Decoding empty symbol table (code after)" ([], [0x78, 0x45]) (readSymbolTable (symbolTableEnd ++ [0x78, 0x45]))]
...
testHello0x37_SymbolTable = TestList    [ myAssertEqual ("Encoding " ++ title) encoded (writeSymbolTable symtab)
                                        , myAssertEqual ("Decoding " ++ title ++ " (nothing after)") (symtab, []) (readSymbolTable encoded)
                                        , myAssertEqual ("Decoding " ++ title ++ " (code after)") (symtab, [0x78, 0x45]) (readSymbolTable (encoded ++ [0x78, 0x45]))]
